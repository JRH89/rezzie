...
Current implementation: `apps/extension` builds a Manifest V3 side panel with `activeTab`, `scripting`, `sidePanel`, `storage`, and `downloads`. It has a typed message protocol, guarded visible-page extraction with Greenhouse/Lever/Ashby/Workday selectors and generic fallback, saved-resume selection, shared-credit or request-scoped BYOK tailoring, and explicit TXT/PDF/DOCX downloads. Email/password uses Firebase's extension entry point. Google sign-in opens a hosted `https://rezzie.org/extension-auth` tab so `signInWithPopup` is run from a real user click. The extension generates a one-time request ID and accepts the returned token only from the hosted origin when that pending ID matches. Add `https://rezzie.org` to Firebase Authorized domains before enabling it.
