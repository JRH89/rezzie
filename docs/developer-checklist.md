...
| Engineering | Preserve section-heading structure in tailored output | complete | API normalizes known bullet-prefixed headings and the editor recognizes legacy drafts; API/document and web formatting regression tests cover `SELECTED PROJECTS`. |
...
| Platform | Configure Cloudflare Worker frontend | in progress | Root `wrangler.jsonc` supplies static assets, SPA fallback, and `apps/web/seo-worker.ts` for crawler-visible per-route metadata; set production build variables and attach `rezzie.org` |
...
| QA | Run local API/web checks | complete | 2026-09-15: 86 API tests and API Ruff passed with test-safe host/origin settings. Web checks previously passed 2026-09-12. |
| Engineering | Preserve usable resume document structure | in progress | Rich editor formatting and safe HTTP(S), `www.`, and `mailto:` hyperlinks carry into matching DOCX/PDF outputs, even when an export starts from plain text. When the original DOCX remains in the current browser session, source-DOCX export scans and patches it in memory, preserving document setup and paragraph/list styles; API format and route tests cover this path. Exact saved-source persistence remains pending private original-file storage and consent; see `docs/document-fidelity.md` |
...
| Engineering | Cover letters and Trusted Source evidence | complete | Standalone or paired truth-preserving cover letters, one-credit letter billing, all-user owned GitHub/portfolio source connection, subscriber-included source use, non-subscriber one-credit source supplement, SSRF-safe fetcher, and SQLite-backed request limits; `docs/trusted-sources.md` |
