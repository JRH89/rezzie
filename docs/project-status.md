...
| Chrome extension | in progress | Manifest V3 side panel builds locally with guarded active-tab JD extraction, Firebase email/password and Google relay authentication, session-only token storage, saved-resume selection, shared-credit tailoring, and TXT/PDF/DOCX export; Firebase extension-ID allow-list and Store testing remain external |
