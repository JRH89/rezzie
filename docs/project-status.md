...
| Resume document fidelity | in progress | Rich-editor formatting and safe HTTP(S), `www.`, and `mailto:` links survive DOCX/PDF exports, including plain-text/legacy drafts. A current-session DOCX download now patches the uploaded source document in memory, retaining document setup and paragraph/list styles without retaining the original file. PDF remains best-effort; durable source-file retention for saved resumes still requires private object storage and explicit consent. See `docs/document-fidelity.md` |
