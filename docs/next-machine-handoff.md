...
`wrangler.jsonc` uses `apps/web/seo-worker.ts` in front of the static asset binding with `run_worker_first: true`. It rewrites canonical, title, description, Open Graph, Twitter, and article metadata for public routes before the SPA loads, then delegates assets to Cloudflare. Keep that worker and `apps/web/src/MarketingPages.tsx` metadata aligned when adding a new public page.
