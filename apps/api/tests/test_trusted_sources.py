...
class FakeFetcher(TrustedSourceFetcher):
    async def fetch(self, url: str) -> tuple[str, str]:
        return (
            url,
            "Built and maintained a public portfolio project with documented implementation details.",
        )


@pytest.mark.asyncio
async def test_trusted_sources_are_available_to_all_users_and_remain_private(
    tmp_path: object,
) -> None:
    billing = BillingRepository(
        f"sqlite:///{tmp_path}/sources.db", bootstrap_schema=True
    )
    ...
    source = await service.add(
        "user-a", url="https://github.com/example", label="GitHub"
    )
...
def test_rate_limiter_blocks_after_window_limit(tmp_path: object) -> None:
    billing = BillingRepository(
        f"sqlite:///{tmp_path}/limits.db", bootstrap_schema=True
    )
