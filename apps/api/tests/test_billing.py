...
def test_credit_is_consumed_and_refundable(tmp_path: object) -> None:
    ...
    with pytest.raises(HTTPException) as error:
        repository.consume_credit("user-1")
...
def test_webhook_grants_a_credit_pack_once(
    ...
    service = StripeBillingService(
        Settings(stripe_webhook_secret="whsec_test"), repository
    )
    event = {
        "id": "evt_purchase",
    ...
    monkeypatch.setattr(
        "rezzie.billing.stripe.Webhook.construct_event", lambda *_: event
    )
...
def test_paid_invoice_resets_monthly_credit_allowance(
    tmp_path: object, monkeypatch: pytest.MonkeyPatch
) -> None:
    repository = BillingRepository(
        f"sqlite:///{tmp_path}/billing.db", bootstrap_schema=True
    ...
    service = StripeBillingService(
        Settings(
            stripe_webhook_secret="whsec_test", stripe_subscription_monthly_credits=20
        ),
        repository,
    )
    event = {
...
def test_webhook_handles_stripe_resource_and_retries_previously_unhandled_event(
    tmp_path: object, monkeypatch: pytest.MonkeyPatch
) -> None:
    repository = BillingRepository(
        f"sqlite:///{tmp_path}/billing.db", bootstrap_schema=True
...
def test_credit_checkout_uses_selected_pack_quantity(
    tmp_path: object, monkeypatch: pytest.MonkeyPatch
) -> None:
    repository = BillingRepository(
        f"sqlite:///{tmp_path}/billing.db", bootstrap_schema=True
    ...
    service = StripeBillingService(
        Settings(
            app_url="https://rezzie.example",
            stripe_secret_key="sk_test",
            stripe_credit_packs='{"price_pack":20}',
        ),
        repository,
    )
    captured: dict[str, object] = {}
    monkeypatch.setattr(
        "rezzie.billing.stripe.Customer.create", lambda **_: {"id": "cus_1"}
    )
    monkeypatch.setattr(
        "rezzie.billing.stripe.checkout.Session.create",
        lambda **kwargs: (
            captured.update(kwargs) or SimpleNamespace(url="https://checkout.example")
        ),
    )

    assert (
        service.checkout(
            "user-1", CheckoutRequest(kind="credits", price_id="price_pack", quantity=3)
        )
    ...
    assert (
        ...
        == "https://rezzie.example/#workspace?checkout=success&session_id={CHECKOUT_SESSION_ID}"
    )
    assert (
        captured["cancel_url"] == "https://rezzie.example/#workspace?checkout=cancelled"
    )


@pytest.mark.parametrize("quantity", [0, 11])
def test_credit_checkout_rejects_out_of_range_quantity(
    tmp_path: object, quantity: int
) -> None:
    repository = BillingRepository(
        f"sqlite:///{tmp_path}/billing.db", bootstrap_schema=True
    )
    service = StripeBillingService(
        Settings(stripe_secret_key="sk_test", stripe_credit_packs='{"price_pack":20}'),
        repository,
    )
    ...
    with pytest.raises(HTTPException, match="quantity must be between 1 and 10"):
        service.checkout(
            "user-1",
            CheckoutRequest(kind="credits", price_id="price_pack", quantity=quantity),
        )


def test_credit_checkout_returns_safe_error_when_stripe_rejects_configuration(
    tmp_path: object, monkeypatch: pytest.MonkeyPatch
) -> None:
    repository = BillingRepository(
        f"sqlite:///{tmp_path}/billing.db", bootstrap_schema=True
    ...
    monkeypatch.setattr(
        "rezzie.billing.stripe.Customer.create",
        lambda **_: (_ for _ in ()).throw(stripe.error.AuthenticationError("bad key")),
    )

    with pytest.raises(
        HTTPException, match="Verify the server's live Stripe key"
    ) as error:
        service.checkout(
            "user-1", CheckoutRequest(kind="credits", price_id="price_pack")
        )
