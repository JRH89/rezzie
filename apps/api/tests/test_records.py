...
def test_imported_facts_require_confirmation_before_becoming_tailoring_source(
    tmp_path: object,
) -> None:
    billing = BillingRepository(
        f"sqlite:///{tmp_path}/records.db", bootstrap_schema=True
    )
    ...
    records.update_fact(
        ...
        text=confirmed.text,
        status="confirmed",
        evidence_note="Confirmed by candidate.",
    )
    source = records.confirmed_source("candidate-1", record.id)
    assert confirmed.text in source


def test_candidate_can_add_and_confirm_a_fact_without_cross_user_access(
    tmp_path: object,
) -> None:
    billing = BillingRepository(
        f"sqlite:///{tmp_path}/records.db", bootstrap_schema=True
    ...
    record = records.create(
        "candidate-1",
        "Primary record",
        "A sufficiently long original resume source line for this test.",
    )
    fact = records.add_fact(
        "candidate-1",
        record.id,
        fact_type="skill",
        text="TypeScript",
