...
def docx_style_profile(document: Document) -> dict[str, str | float | bool]:
    ...
    font_names = [
        ...
        if run.font.name in ALLOWED_FONT_FAMILIES
    ]
    font_family = (
        Counter(font_names).most_common(1)[0][0]
        if font_names
        else normal.font.name
        if normal.font.name in ALLOWED_FONT_FAMILIES
        else "Aptos"
    )
    body_paragraphs = [
        paragraph
        for index, paragraph in enumerate(paragraphs)
        if index > 1 and not is_section_heading(paragraph.text)
    ]
    body_sizes = [
        size
        for paragraph in body_paragraphs
        for run in paragraph.runs
        if (size := _point_size(run.font.size)) is not None
    ]
    body_size = (
        min(14, max(8, sum(body_sizes) / len(body_sizes))) if body_sizes else 10.5
    )
    heading_paragraphs = [
        paragraph for paragraph in paragraphs if is_section_heading(paragraph.text)
    ]
    heading_sizes = [
        size
    ...
    heading_size = (
        min(16, max(9, sum(heading_sizes) / len(heading_sizes)))
        if heading_sizes
        else max(10.5, body_size)
    )
    name_sizes = (
        [
            size
    ...
    name_size = (
        min(28, max(12, max(name_sizes))) if name_sizes else max(18, heading_size + 5)
    )
    emphasis_candidates = [
        paragraph
        for paragraph in body_paragraphs
    ...
    emphasize_role_lines = any(
        sum(len(run.text) for run in paragraph.runs if run.bold)
        >= len(paragraph.text.strip()) * 0.6
        for paragraph in emphasis_candidates
        if paragraph.text.strip()
    )
    italic_metadata = any(
        bool(run.italic) and re.search(r"\b(?:19|20)\d{2}\b", paragraph.text)
        for paragraph in paragraphs
        for run in paragraph.runs
    )
    return {
        ...
        "heading_uppercase": all(
            paragraph.text.strip() == paragraph.text.strip().upper()
            for paragraph in heading_paragraphs
        )
...
def docx_inline_runs(paragraph: DocxParagraph) -> list[tuple[DocxRun, str | None]]:
    ...
    for child in paragraph._p:
        ...
        try:
            relationship = (
                paragraph.part.rels[relationship_id] if relationship_id else None
            )
...
def docx_editor_html(document: Document) -> str:
    ...
    for paragraph in document.paragraphs:
        if not paragraph.text.strip():
            continue
        is_bullet = (
            "List Bullet" in (paragraph.style.name or "")
...
def _linkify_run(run: TextRun) -> list[TextRun]:
    ...
    for match in URL_PATTERN.finditer(run.text):
        ...
        href = (
            f"https://{visible_url}"
            if visible_url.lower().startswith("www.")
            else visible_url
        )
        linked_runs.append(
    ...
    if cursor < len(run.text):
        linked_runs.append(
            TextRun(run.text[cursor:], run.bold, run.italic, run.underline)
        )
    return linked_runs
...
def _linkify_blocks(blocks: list[ResumeBlock]) -> list[ResumeBlock]:
    for block in blocks:
        block.runs = [
            linked_run for run in block.runs for linked_run in _linkify_run(run)
        ]
    return blocks
    ...
    for paragraph in document.paragraphs:
        for child in paragraph._p:
            ...
            try:
                ...
                href = None
            label = "".join(
                DocxRun(run, paragraph).text for run in child.findall(qn("w:r"))
            ).strip()
            pair = (label, href or "")
...
def _preserve_source_link_labels(
    blocks: list[ResumeBlock], links: list[tuple[str, str]]
) -> list[ResumeBlock]:
...
class ResumeHtmlParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag in self._block_tags:
            ...
            self._styles.append(
                (tag, safe_link(attributes.get("href")) if tag == "a" else None)
            )
    ...
    def _append(self, text: str) -> None:
        ...
        href = next(
            (value for tag, value in reversed(self._styles) if tag == "a"), None
        )
        run = TextRun(
            text=text,
            bold=bool(tags & {"b", "strong"}),
            italic=bool(tags & {"i", "em"}),
            underline="u" in tags,
            href=href,
        )
        if (
            self._current.runs
            and self._current.runs[-1].bold == run.bold
            and self._current.runs[-1].italic == run.italic
            and self._current.runs[-1].underline == run.underline
            and self._current.runs[-1].href == run.href
        ):
            prior = self._current.runs[-1]
            self._current.runs[-1] = TextRun(
                prior.text + run.text,
                prior.bold,
                prior.italic,
                prior.underline,
...
def editor_html_matches_resume_text(resume_html: str | None, resume_text: str) -> bool:
    ...
    return (
        ...
        and collapsed_html == collapsed_text
    )
...
def document_blocks(resume_text: str, resume_html: str | None) -> list[ResumeBlock]:
    ...
    blocks = [
        ResumeBlock(
            ...
            runs=[
                TextRun(
                    line[2:].strip() if line.startswith(("- ", "* ", "â€¢ ")) else line
                )
            ],
            alignment="center" if index < 2 else "left",
        )
        if not line.startswith(("- ", "* ", "â€¢ "))
        else ResumeBlock(kind="li", runs=[TextRun(line[2:].strip())])
...
class DocumentService:
    ...
    async def extract(self, file: UploadFile) -> ExtractedDocument:
        ...
        if not document_type:
            raise HTTPException(
                status_code=422, detail="Use a .txt, .md, .pdf, or .docx document."
            )
        ...
        try:
            ...
            if document_type == "text":
                ...
                text, page_count, style_profile, entry_lines, editor_html = (
                    paragraph_text(document),
                    None,
                    docx_style_profile(document),
                    docx_entry_lines(document),
                    docx_editor_html(document),
                )
        except Exception as error:
            raise HTTPException(
                status_code=422, detail="That document could not be read."
            ) from error
        text = text.strip()
        if len(text) < 50:
            raise HTTPException(
                status_code=422,
                detail="The document is too short or has no readable text.",
            )
    ...
    def validate_docx_source(self, data: bytes) -> None:
        ...
        try:
            ...
            raise HTTPException(
                status_code=422, detail="The original DOCX could not be opened."
            ) from error
    ...
    def _scan(self, data: bytes) -> None:
        if not self._settings.clamav_host:
            if self._settings.environment == "development":
                return
            raise HTTPException(
                status_code=503, detail="Document scanning is not configured."
            )
        try:
            with socket.create_connection(
                (self._settings.clamav_host, self._settings.clamav_port), timeout=5
            ) as client:
                client.sendall(b"zINSTREAM\0")
                client.sendall(
                    len(data).to_bytes(4, "big") + data + (0).to_bytes(4, "big")
                )
                response = client.recv(4096).decode("utf-8", errors="replace")
        except OSError as error:
            raise HTTPException(
                status_code=503, detail="Document scanning is unavailable."
            ) from error
        if "OK" not in response or "FOUND" in response:
            raise HTTPException(
                status_code=422, detail="The document did not pass the malware scan."
            )
...
class ResumeExportService:
    ...
    def render_docx(
        self, resume_text: str, target_page_count: int | None = None
    ) -> bytes:
        document = Document()
    ...
    def render_pdf(
        ...
        for scale in scales:
            ...
            if (
                ...
                or len(PdfReader(io.BytesIO(rendered)).pages) <= target_page_count
            ):
                return rendered
        return rendered
...
RESUME_TEMPLATES = {
    ...
    "classic": ResumeTemplate(
        "Times New Roman", "Times-Roman", 10.5, 19, 11, 13.5, 0.75
    ),
...
def source_template(template: ResumeTemplate, profile) -> ResumeTemplate:
    ...
    value = (
        ...
        else lambda key, default: getattr(profile, key, default)
    )
...
def _alignment(value: str) -> WD_ALIGN_PARAGRAPH:
    return {"center": WD_ALIGN_PARAGRAPH.CENTER, "right": WD_ALIGN_PARAGRAPH.RIGHT}.get(
        value, WD_ALIGN_PARAGRAPH.LEFT
    )
...
class RichResumeExportService(ResumeExportService):
    ...
    def render_docx(
        ...
        template = (
            source_template(resume_template(template_id), style_profile)
            if template_id == "source"
            else resume_template(template_id)
        )
        ...
        normal.paragraph_format.line_spacing = Pt(
            template.line_height - (0.8 if compact else 0)
        )
        normal.paragraph_format.space_after = Pt(2 if compact else 4)
        ...
        for index, block in enumerate(document_blocks(resume_text, resume_html)):
            ...
            kind = (
                ...
                if block.kind == "h3"
                or (block.kind == "p" and is_section_heading(text))
                else block.kind
            )
            paragraph = document.add_paragraph(
                style="List Bullet" if kind == "li" else None
            )
            paragraph.alignment = _alignment(
                "center" if kind == "h1" and index == 0 else block.alignment
            )
            paragraph.paragraph_format.space_before = Pt(7 if kind == "h3" else 0)
            paragraph.paragraph_format.space_after = Pt(
                3 if kind == "h3" else 2 if compact else 4
            )
            size = (
                template.name_size
                if kind == "h1"
                else template.heading_size
                if kind == "h3"
                else template.body_size - (0.5 if compact else 0)
            )
    ...
    def render_source_docx(
        self, source_docx: bytes, resume_text: str, resume_html: str | None = None
    ) -> bytes:
        ...
        if not paragraphs:
            raise HTTPException(
                status_code=422,
                detail="The original DOCX has no editable body paragraphs.",
            )
    ...
    @classmethod
    def _source_paragraph_templates(
        cls, paragraphs: list[DocxParagraph]
    ) -> dict[str, DocxParagraph]:
    ...
    @staticmethod
    def _clone_paragraph_after(
        template: DocxParagraph, anchor: DocxParagraph
    ) -> DocxParagraph:
    ...
    def _replace_source_paragraph(
        self, paragraph: DocxParagraph, block: ResumeBlock, template: DocxParagraph
    ) -> None:
    ...
    @staticmethod
    def _render_rich_pdf(
        blocks: list[ResumeBlock], template: ResumeTemplate, scale: float
    ) -> bytes:
        output = io.BytesIO()
        ...
        name_style = ParagraphStyle(
            "RichResumeName",
            parent=styles["Title"],
            fontName=template.pdf_font_name,
            fontSize=template.name_size * scale,
            leading=(template.name_size + 3) * scale,
            alignment=1,
            spaceAfter=4 * scale,
        )
        heading_style = ParagraphStyle(
            ...
            parent=styles["Heading2"],
            fontName=template.pdf_font_name,
            fontSize=template.heading_size * scale,
            leading=(template.heading_size + 2) * scale,
            spaceBefore=9 * scale,
            spaceAfter=4 * scale,
        )
        body_style = ParagraphStyle(
            ...
            parent=styles["Normal"],
            fontName=template.pdf_font_name,
            fontSize=template.body_size * scale,
            leading=template.line_height * scale,
            spaceAfter=3 * scale,
        )
        story = []
        for index, block in enumerate(blocks):
            ...
            for run in block.runs:
                value = escape(run.text).replace("\n", "<br/>")
                if run.href:
                    value = f'<a href="{escape(run.href, quote=True)}">{value}</a>'
                if run.underline:
                    value = f"<u>{value}</u>"
                if run.italic:
                    value = f"<i>{value}</i>"
                if run.bold or kind in {"h1", "h3"}:
                    value = f"<b>{value}</b>"
                fragments.append(value)
            if kind == "li":
                fragments.insert(0, "&bull; ")
            style = (
                name_style
                if kind == "h1" and index == 0
                else heading_style
                if kind == "h3"
                else body_style
            )
