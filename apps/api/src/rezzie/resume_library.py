...
class ResumeVersion(Base):
    ...
    resume_id: Mapped[str] = mapped_column(
        ForeignKey("saved_resumes.id", ondelete="CASCADE"), index=True
    )
...
class ResumeLibraryService:
    ...
    def create(
        self, user_id: str, *, label: str, source_text: str
    ) -> tuple[SavedResume, ResumeVersion]:
        limit = self._limit(user_id)
        with self._sessions.begin() as session:
            count = (
                session.scalar(
                    select(func.count())
                    .select_from(SavedResume)
            ...
            if count >= limit:
                raise HTTPException(
                    status_code=409,
                    detail=f"Your current plan can save up to {limit} resume{'s' if limit != 1 else ''}. Delete one or add credits to save more.",
                )
            resume = SavedResume(id=str(uuid4()), user_id=user_id, label=label.strip())
            version = ResumeVersion(
                id=str(uuid4()), resume_id=resume.id, source_text=source_text
            )
    ...
    def list(self, user_id: str) -> list[tuple[SavedResume, ResumeVersion]]:
        with self._sessions() as session:
            ...
            return [
                (resume, self._latest_version(session, resume.id)) for resume in resumes
            ]
    ...
    def add_version(
        self, user_id: str, resume_id: str, source_text: str
    ) -> tuple[SavedResume, ResumeVersion]:
        with self._sessions.begin() as session:
            resume = self._owned_resume(session, user_id, resume_id)
            version = ResumeVersion(
        ...
        with self._sessions.begin() as session:
            count = (
                session.scalar(
                    ...
                    .where(SavedTailoringDraft.user_id == user_id)
                )
                or 0
            )
            if count >= limit:
                raise HTTPException(
                    status_code=409,
                    detail=f"Your current plan can save up to {limit} tailored drafts. Delete one or add credits to save more.",
                )
            if resume_id:
        ...
        with self._sessions() as session:
            return list(
                session.scalars(
                    ...
                    .where(SavedTailoringDraft.user_id == user_id)
                    .order_by(SavedTailoringDraft.created_at.desc())
                )
            )
    ...
    def _latest_version(session, resume_id: str) -> ResumeVersion:
        ...
        if not version:
            raise HTTPException(
                status_code=500, detail="Saved resume has no source version."
            )
        return version
    ...
    def _owned_draft(session, user_id: str, draft_id: str) -> SavedTailoringDraft:
        ...
        if not draft or draft.user_id != user_id:
            raise HTTPException(
                status_code=404, detail="Saved tailored draft not found."
            )
        return draft
