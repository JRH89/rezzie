...
def is_configured_admin(settings: Settings, identity: VerifiedIdentity | None) -> bool:
    ...
    configured_email = (
        settings.admin_email.strip().casefold() if settings.admin_email else None
    )
    return bool(
        identity
...
def verified_identity(
    ...
    development_user_email: str | None = None,
) -> VerifiedIdentity | None:
    if settings.environment == "development":
        if development_user_id and len(development_user_id) <= 128:
            email = (
                development_user_email.strip().casefold()
                if development_user_email and len(development_user_email) <= 320
                else None
            )
            return VerifiedIdentity(
                user_id=development_user_id, email=email, email_verified=bool(email)
            )
        return None
    ...
    try:
        signing_key = (
            _jwks_client(settings.oidc_jwks_url).get_signing_key_from_jwt(token).key
        )
        claims = jwt.decode(
            token,
            signing_key,
    ...
    email = (
        email_claim.strip().casefold()
        if isinstance(email_claim, str) and len(email_claim) <= 320
        else None
    )
    return VerifiedIdentity(
        user_id=subject,
...
def verified_user_id(
    settings: Settings, authorization: str | None, development_user_id: str | None
) -> str | None:
