...
class SupportRepository:
    ...
    def create(
        self, user_id: str, *, subject: str, category: str, message: str
    ) -> SupportTicket:
    ...
    def reply(
        self, ticket_id: str, *, author_id: str, author_role: str, body: str
    ) -> SupportTicket:
        with self._sessions.begin() as session:
            ticket = session.get(SupportTicket, ticket_id)
            if not ticket:
            ...
            if author_role == "customer" and ticket.status in {"resolved", "closed"}:
                ticket.status = "open"
            return ticket
