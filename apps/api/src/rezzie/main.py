...
from .schemas import (
    ...
    CoverLetterCareerRecordRequest,
    CoverLetterRequest,
    CoverLetterResult,
    CreditBalance,
    ImportResponse,
...
billing_repository = BillingRepository(
    settings.database_url, bootstrap_schema=settings.environment == "development"
)
...
importer, tailoring_service = (
    JobDescriptionImporter(settings),
    TailoringService(
        AnthropicProvider(
...
def require_admin(
    ...
    development_user_email: str | None,
) -> str:
    identity = verified_identity(
        settings, authorization, development_user_id, development_user_email
    )
    if not identity:
        raise HTTPException(status_code=401, detail="Authentication is required.")
...
def saved_resume_response(
    resume: SavedResume, version: ResumeVersion
) -> SavedResumeResponse:
...
def support_ticket_response(
    ticket: SupportTicket, *, include_messages: bool = False
) -> SupportTicketResponse:
...
async def import_file(
    ...
    return ImportResponse(
        ...
        style_profile=document.style_profile,
        entry_lines=document.entry_lines or [],
...
def save_resume(
    request: SavedResumeCreate,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
) -> SavedResumeResponse:
    resume, version = resume_library.create(
        require_user(authorization, x_rezzie_user_id),
        label=request.label,
...
def list_saved_resumes(
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
) -> list[SavedResumeResponse]:
    return [
        saved_resume_response(resume, version)
        for resume, version in resume_library.list(
            require_user(authorization, x_rezzie_user_id)
        )
    ]
...
def get_saved_resume(
    ...
    resume, version = resume_library.get(
        require_user(authorization, x_rezzie_user_id), resume_id
    )
    return saved_resume_response(resume, version)
...
def add_resume_version(
    ...
    resume, version = resume_library.add_version(
        require_user(authorization, x_rezzie_user_id), resume_id, request.source_text
    )
    return saved_resume_response(resume, version)
...
@app.post(
    "/api/v1/trusted-sources", response_model=TrustedSourceResponse, status_code=201
)
async def add_trusted_source(
    ...
    if not request.ownership_attested:
        raise HTTPException(
            status_code=422,
            detail="Confirm that you own or are authorized to use this public source.",
        )
    source = await trusted_source_service.add(
        user_id, url=str(request.url), label=request.label.strip()
    )
...
def list_trusted_sources(
    ...
    return [
        trusted_source_response(source)
        for source in trusted_sources.list(
            require_user(authorization, x_rezzie_user_id)
        )
    ]
...
@app.post(
    ...
    response_model=SavedTailoringDraftResponse,
    status_code=201,
)
def save_tailoring_draft(
    request: SavedTailoringDraftCreate,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
) -> SavedTailoringDraftResponse:
...
def list_tailoring_drafts(
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
) -> list[SavedTailoringDraftResponse]:
    return [
        saved_draft_response(draft)
        for draft in resume_library.list_drafts(
            require_user(authorization, x_rezzie_user_id)
        )
    ]
...
@app.get(
    "/api/v1/tailoring-drafts/{draft_id}", response_model=SavedTailoringDraftResponse
)
def get_tailoring_draft(
    ...
    return saved_draft_response(
        resume_library.draft(require_user(authorization, x_rezzie_user_id), draft_id)
    )


@app.delete("/api/v1/tailoring-drafts/{draft_id}", status_code=204)
def delete_tailoring_draft(
    draft_id: str,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
) -> Response:
...
def export_resume(
    ...
    resume_html = (
        request.resume_html
        if editor_html_matches_resume_text(request.resume_html, request.resume_text)
        else None
    )
    content = resume_export_service.render_docx(
        editor_html_to_text(resume_html, request.resume_text),
        resume_html,
...
async def export_source_docx(
    ...
    require_user(authorization, x_rezzie_user_id)
    if (
        original_docx.content_type
        != "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    ):
        raise HTTPException(
            status_code=422,
            detail="Upload the original DOCX file to preserve its layout.",
        )
    ...
    export_html = (
        resume_html
        if editor_html_matches_resume_text(resume_html, resume_text)
        else None
    )
    content = resume_export_service.render_source_docx(
        source_docx, editor_html_to_text(export_html, resume_text), export_html
    )
    return Response(
        content=content,
        media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
...
def export_resume_pdf(
    ...
    content = resume_export_service.render_pdf(
        editor_html_to_text(resume_html, request.resume_text),
        resume_html,
        request.target_page_count,
        request.template_id,
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": 'attachment; filename="rezzie-tailored-resume.pdf"',
            "Cache-Control": "no-store",
        },
    )
...
def create_career_record(
    request: CareerRecordCreate,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
) -> CareerRecordResponse:
    user_id = require_user(authorization, x_rezzie_user_id)
    return career_record_response(
        user_id, career_records.create(user_id, request.label, request.source_text)
    )
...
def list_career_records(
    ...
    user_id = require_user(authorization, x_rezzie_user_id)
    return [
        career_record_response(user_id, record)
        for record in career_records.list_records(user_id)
    ]


@app.get("/api/v1/career-records/{record_id}", response_model=CareerRecordResponse)
def get_career_record(
    record_id: str,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
) -> CareerRecordResponse:
...
@app.patch(
    "/api/v1/career-records/{record_id}/facts/{fact_id}",
    response_model=CareerFactResponse,
)
def update_career_fact(
    record_id: str,
    fact_id: str,
    request: CareerFactUpdate,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
) -> CareerFactResponse:
    user_id = require_user(authorization, x_rezzie_user_id)
    fact = career_records.update_fact(
        user_id,
        record_id,
        fact_id,
        text=request.text,
...
def add_career_fact(
    record_id: str,
    request: CareerFactCreate,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
) -> CareerFactResponse:
    user_id = require_user(authorization, x_rezzie_user_id)
    fact = career_records.add_fact(
        user_id,
        record_id,
...
async def tailor(
    ...
    if request.external_source_ids:
        if not user_id:
            raise HTTPException(
                status_code=401,
                detail="Authentication is required for Trusted Sources.",
            )
        rate_limiter.enforce(
            "source-backed-tailoring-user", user_id, limit=12, seconds=900
        )
        ...
        sources = []
    try:
        return await tailoring_service.tailor(
            request, user_id, sources, admin_override=admin_override
        )
        ...
        return JSONResponse(
            ...
            headers=cors_error_headers(http_request),
        )


@app.post("/api/v1/cover-letters", response_model=CoverLetterResult)
async def create_cover_letter(
    ...
    if not identity:
        raise HTTPException(
            status_code=401,
            detail="Authentication is required to create a cover letter.",
        )
    user_id = identity.user_id
    admin_override = is_configured_admin(settings, identity)
    if request.external_source_ids:
        rate_limiter.enforce(
            "source-backed-cover-letter-user", user_id, limit=12, seconds=900
        )
        sources = trusted_sources.selected(user_id, request.external_source_ids)
    else:
        rate_limiter.enforce("cover-letter-user", user_id, limit=12, seconds=900)
        sources = []
    try:
        return await tailoring_service.cover_letter(
            request, user_id, sources, admin_override=admin_override
        )
    except ValueError as error:
        logger.warning("Cover-letter provider request failed: %s", error)
        return JSONResponse(
            status_code=503,
            content={"detail": str(error)},
            headers=cors_error_headers(http_request),
...
@app.post("/api/v1/cover-letters/career-record", response_model=CoverLetterResult)
async def create_career_record_cover_letter(
    request: CoverLetterCareerRecordRequest,
    http_request: Request,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
    x_rezzie_user_email: str | None = Header(default=None),
) -> CoverLetterResult:
    """Generate a letter from confirmed facts, never from an unverified draft."""
    identity = verified_identity(
        settings, authorization, x_rezzie_user_id, x_rezzie_user_email
    )
    if not identity:
    ...
    try:
        ...
        logger.warning("Career-record cover-letter provider request failed: %s", error)
        return JSONResponse(
            status_code=503,
            content={"detail": str(error)},
            headers=cors_error_headers(http_request),
...
async def tailor_career_record(
    request: TailorCareerRecordRequest,
    http_request: Request,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
    x_rezzie_user_email: str | None = Header(default=None),
...
def create_checkout(
    ...
    if is_configured_admin(settings, identity):
        raise HTTPException(
            status_code=403,
            detail="Your administrator account has unlimited internal access and does not need checkout.",
        )
    user_id = identity.user_id
...
def billing_balance(
    ...
    status, subscription_remaining, purchased_credits = billing_repository.balance(
        identity.user_id
    )
    return CreditBalance(
...
def billing_portal(
    ...
    return {
        "url": billing_service.portal(require_user(authorization, x_rezzie_user_id))
    }
...
@app.post(
    "/api/v1/support/tickets", response_model=SupportTicketResponse, status_code=201
)
...
def list_support_tickets(
    ...
    return [
        support_ticket_response(ticket)
        for ticket in support_tickets.list_for_user(
            require_user(authorization, x_rezzie_user_id)
        )
    ]
...
def get_support_ticket(
    ...
    return support_ticket_response(
        support_tickets.get_for_user(
            require_user(authorization, x_rezzie_user_id), ticket_id
        ),
        include_messages=True,
    )
...
@app.post(
    "/api/v1/support/tickets/{ticket_id}/messages", response_model=SupportTicketResponse
)
def reply_to_support_ticket(
    ...
    return support_ticket_response(
        support_tickets.reply(
            ticket_id, author_id=user_id, author_role="customer", body=request.message
        ),
        include_messages=True,
    )
...
@app.get(
    "/api/v1/admin/support/tickets/{ticket_id}", response_model=SupportTicketResponse
)
def get_admin_support_ticket(
    ...
    require_admin(authorization, x_rezzie_user_id, x_rezzie_user_email)
    return support_ticket_response(
        support_tickets.get_any(ticket_id), include_messages=True
    )
...
def reply_to_admin_support_ticket(
    ticket_id: str,
    request: SupportTicketReplyCreate,
    authorization: str | None = Header(default=None),
    x_rezzie_user_id: str | None = Header(default=None),
    ...
    return support_ticket_response(
        support_tickets.reply(
            ticket_id, author_id=admin_id, author_role="staff", body=request.message
        ),
        include_messages=True,
    )
...
def update_admin_support_ticket(
    ...
    require_admin(authorization, x_rezzie_user_id, x_rezzie_user_email)
    return support_ticket_response(
        support_tickets.update_status(ticket_id, request.status), include_messages=True
    )
