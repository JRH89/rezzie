...
class CareerFact(Base):
    ...
    record_id: Mapped[str] = mapped_column(
        ForeignKey("career_records.id", ondelete="CASCADE"), index=True
    )
...
class CareerRecordRepository:
    ...
    def create(self, user_id: str, label: str, source_text: str) -> CareerRecord:
        record = CareerRecord(
            id=str(uuid4()), user_id=user_id, label=label, source_text=source_text
        )
    ...
    def list_records(self, user_id: str) -> list[CareerRecord]:
        with self._sessions() as session:
            return list(
                session.scalars(
                    select(CareerRecord)
                    .where(CareerRecord.user_id == user_id)
                    .order_by(CareerRecord.updated_at.desc())
                )
            )
    ...
    def facts(self, user_id: str, record_id: str) -> list[CareerFact]:
        ...
        with self._sessions() as session:
            return list(
                session.scalars(
                    select(CareerFact)
                    .where(CareerFact.record_id == record_id)
    ...
    def add_fact(
        ...
        *,
        ...
        fact = CareerFact(
            ...
            fact_type=fact_type,
            text=text,
            source_excerpt="Added directly by the candidate.",
            status="needs_review",
    ...
    def confirmed_source(self, user_id: str, record_id: str) -> str:
        ...
        confirmed = [
            fact
            for fact in self.facts(user_id, record_id)
            if fact.status == "confirmed"
        ]
        if not confirmed:
            raise HTTPException(
                status_code=422,
                detail="Confirm at least one Career Record fact before tailoring from it.",
            )
        sections: Iterable[str] = [
            f"CAREER RECORD: {record.label}",
            "CONFIRMED FACTS:",
            *(f"- {fact.text}" for fact in confirmed),
        ]
        return "\n".join(sections)
