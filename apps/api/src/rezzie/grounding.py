...
YEAR_RANGE = re.compile(
    r"\b(19\d{2}|20\d{2})\s*(?:-|–|—|to)\s*((?:19|20)\d{2}|present|current|now)\b",
    re.IGNORECASE,
)
...
def unsupported_quantitative_claims(
    source_resume: str, tailored_resume: str, reference_date: date
) -> set[str]:
...
def remove_unsupported_quantitative_lines(
    source_resume: str, tailored_resume: str, reference_date: date | None = None
) -> str:
    ...
    kept_lines = [
        ...
        if not unsupported_quantitative_claims(source_resume, line, as_of)
    ]
...
def assert_grounded(
    ...
    invented_numbers = unsupported_quantitative_claims(
        source_resume, tailored_resume, reference_date or current_date()
    )
    if invented_numbers:
        raise HTTPException(
            status_code=422,
            detail="The generated draft introduced an unsupported quantitative claim.",
        )
