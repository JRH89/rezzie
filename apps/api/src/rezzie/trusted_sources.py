...
class TrustedSourceRepository:
        ...
        with self._sessions() as session:
            return list(
                session.scalars(
                    ...
                    .where(ExternalSource.user_id == user_id)
                    .order_by(ExternalSource.fetched_at.desc())
                )
            )
    ...
    def selected(self, user_id: str, source_ids: list[str]) -> list[ExternalSource]:
            ...
            sources = list(
                session.scalars(
                    select(ExternalSource).where(
                        ...
                        ExternalSource.id.in_(source_ids),
                    )
                )
            )
        if len(sources) != len(set(source_ids)):
            raise HTTPException(
                status_code=404,
                detail="One or more Trusted Sources could not be found.",
            )
        return sources
            ...
            for redirects_followed in range(self._MAX_REDIRECTS + 1):
                ...
                try:
                    response = await client.get(
                        current_url, headers={"User-Agent": "RezzieTrustedSource/1.0"}
                    )
                    response.raise_for_status()
                except httpx.HTTPError as error:
                    raise HTTPException(
                        status_code=422,
                        detail="Rezzie could not retrieve that public source.",
                    ) from error
                if response.is_redirect:
                    if (
                        redirects_followed == self._MAX_REDIRECTS
                        or not response.headers.get("location")
                    ):
                        raise HTTPException(
                            status_code=422,
                            detail="The source redirected too many times.",
                        )
                ...
                if "text/html" not in response.headers.get("content-type", ""):
                    raise HTTPException(
                        status_code=422,
                        detail="Trusted Sources must be public HTML pages.",
                    )
                if len(response.content) > self._MAX_BYTES:
                    raise HTTPException(
                        status_code=422, detail="That source is too large to import."
                    )
                ...
                if len(extracted) < 50:
                    raise HTTPException(
                        status_code=422,
                        detail="No usable public-source text was found.",
                    )
                return current_url, extracted
        raise HTTPException(
            status_code=422, detail="The source redirected too many times."
        )
...
class TrustedSourceService:
    def __init__(
        self,
        repository: TrustedSourceRepository,
        billing: BillingRepository,
        fetcher: TrustedSourceFetcher | None = None,
    ) -> None:
        self._repository, self._billing, self._fetcher = (
            repository,
            billing,
            fetcher or TrustedSourceFetcher(),
        )

    async def add(self, user_id: str, *, url: str, label: str) -> ExternalSource:
        # Connecting owned public evidence is available to every authenticated
        # user. Billing happens only if a non-subscriber elects to use it in a
        # generation request, never merely for saving a source.
        final_url, extracted = await self._fetcher.fetch(url)
        return self._repository.create(
            user_id,
            label=label,
            url=final_url,
            source_type_value=source_type(final_url),
