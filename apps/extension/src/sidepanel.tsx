...
function Brand() {
  return <div className="brand" aria-label="Rezzie"><span className="brand-mark" aria-hidden="true">R</span><span>rezzie</span></div>;
}

function App() {
  const [token, setToken] = useState<string>(); const [email, setEmail] = useState(""); const [password, setPassword] = useState("");
  const [resumes, setResumes] = useState<Resume[]>([]); const [resumeId, setResumeId] = useState(""); const [job, setJob] = useState<JobSnapshot>(); const [result, setResult] = useState<Result>(); const [error, setError] = useState<string>(); const [busy, setBusy] = useState(false); const [credentialMode, setCredentialMode] = useState<"byok" | "subscription">("subscription"); const [apiKey, setApiKey] = useState("");
  ...
  if (!token) return <main className="panel auth-panel"><Brand /><div className="intro"><span className="eyebrow">YOUR REZZIE ACCOUNT</span><h1>Bring your application work with you.</h1><p>Sign in to use your saved resumes and shared credits.</p></div><button className="button secondary" disabled={busy} onClick={() => void google()}>Continue with Google</button><p className="hint">Google sign-in requires the published extension ID to be allow-listed in Firebase.</p><label>Email<input value={email} onChange={event => setEmail(event.target.value)} type="email" /></label><label>Password<input value={password} onChange={event => setPassword(event.target.value)} type="password" /></label><button className="button" disabled={busy || !email || !password} onClick={() => void signIn()}>{busy ? "Signing in…" : "Sign in"}</button>{error && <p role="alert">{error}</p>}</main>;
  return <main className="panel"><header><Brand /><button className="text-button" onClick={() => { setToken(undefined); void clearSessionToken(); if (auth) void signOut(auth); }}>Sign out</button></header><section className="intro"><span className="eyebrow">TAILOR THIS ROLE</span><h1>Start from what’s already true.</h1><p>Read the job page, choose a saved resume, then review the tailored draft.</p></section><button className="button" disabled={busy} onClick={() => void extract()}>{busy ? "Reading page…" : job ? "Refresh job text" : "Read this job page"}</button>{job && <section className="workflow"><label>Job description<textarea value={job.text} onChange={event => setJob({ ...job, text: event.target.value })} /></label><label>Saved resume<select value={resumeId} onChange={event => setResumeId(event.target.value)}>{resumes.map(resume => <option key={resume.id} value={resume.id}>{resume.label}</option>)}</select></label>{!resumes.length && <p className="hint">Save a resume in Rezzie first, then reopen this panel.</p>}<label>Tailoring access<select value={credentialMode} onChange={event => setCredentialMode(event.target.value as "byok" | "subscription")}><option value="subscription">Use a Rezzie credit</option><option value="byok">Use my Anthropic key</option></select></label>{credentialMode === "byok" && <label>Anthropic API key<input autoComplete="off" onChange={event => setApiKey(event.target.value)} type="password" value={apiKey} /><span className="hint">Used for this request only. It is never saved.</span></label>}<button className="button" disabled={busy || !resumeId || job.text.length < 50 || (credentialMode === "byok" && !apiKey.trim())} onClick={() => void tailor()}>{busy ? "Tailoring…" : credentialMode === "byok" ? "Tailor with my key" : "Tailor with a credit"}</button></section>}{result && <section className="result-card"><div><span className="eyebrow">DRAFT READY</span><h2>Choose your format.</h2></div><textarea value={result.tailored_resume} readOnly /><p className="truth-note">{result.truth_statement}</p><div className="download-actions"><button className="button secondary" disabled={busy} onClick={() => void download("docx")}>DOCX</button><button className="button secondary" disabled={busy} onClick={() => void download("pdf")}>PDF</button><button className="button secondary" disabled={busy} onClick={() => void download("txt")}>TXT</button></div></section>}{error && <p role="alert">{error}</p>}</main>;
}
