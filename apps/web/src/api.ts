...
export type CoverLetterResult = { cover_letter: string; review_items: string[]; truth_statement: string };
export type TailoringResult = { tailored_resume: string; matched_keywords: string[]; review_items: string[]; truth_statement: string; changes: TailoringChange[]; cover_letter?: CoverLetterResult | null };
...
export function createApi(accessToken?: string) {
  ...
  return {
    ...
    tailorCareerRecord: (body: { record_id: string; job_description: string; credential_mode: "byok" | "subscription"; api_key?: string; external_source_ids?: string[]; include_cover_letter?: boolean }) => request<TailoringResult>("/api/v1/tailor/career-record", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
    tailor: (body: { resume_text: string; job_description: string; credential_mode: "byok" | "subscription"; api_key?: string; external_source_ids?: string[]; include_cover_letter?: boolean }) => request<TailoringResult>("/api/v1/tailor", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
    coverLetter: (body: { resume_text: string; job_description: string; credential_mode: "byok" | "subscription"; api_key?: string; external_source_ids?: string[] }) => request<CoverLetterResult>("/api/v1/cover-letters", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
    coverLetterCareerRecord: (body: { record_id: string; job_description: string; credential_mode: "byok" | "subscription"; api_key?: string; external_source_ids?: string[] }) => request<CoverLetterResult>("/api/v1/cover-letters/career-record", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
