...
export function App({ accessToken }: { accessToken?: string }) {
  ...
  const tailor = async () => { setError(undefined); setLoading(true); try { setResult(await api.tailor({ resume_text: resume, job_description: job, credential_mode: credentialMode, api_key: credentialMode === "byok" ? key : undefined })); if (credentialMode === "subscription") setBalance(await api.balance()); } catch (reason) { setError(reason instanceof Error ? reason.message : "Tailoring failed."); } finally { setLoading(false); } };
